...
Displays room name, chapter, difficulty, dynamic difficulty, in-game time, money, kill count, and accuracy. Also shows player position and a live RNG graph for analysis. The Update section shows the current version and notifies when a new release is available.
...
Real-time input visualization overlay. Controller and keyboard/mouse can be toggled independently. Controller mode shows analog stick positions and pad buttons. Keyboard & mouse mode shows mouse movement direction and active key presses.
